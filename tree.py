class Tree:
	...
	def next_best(self, max_move):
		dis_all = None
		for position in self.positions:
			full_move = int(position.split()[-1])
			dis_next = self.positions[position]['dis_next']
			if full_move < max_move and (dis_all is None or dis_all > dis_next):
				dis_all = dis_next
				next = position, self.positions[position]['next']
		return next

	def update(self, position):
		variations = self.book.lookup(chess.Board(position), multipv=True)
		dis_next = None
